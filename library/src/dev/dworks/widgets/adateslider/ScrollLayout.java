...
import dev.dworks.widgets.adateslider.model.Labeler;
import dev.dworks.widgets.adateslider.model.Util;
import dev.dworks.widgets.adateslider.ui.DayTimeLayoutView;
...
public class ScrollLayout extends LinearLayout {
    ...
    public ScrollLayout(Context context, AttributeSet attrs) {
        ...
        mLineHeight = Float.parseFloat(getResources().getString(R.dimen.line_height));
    ...
    public void setTheme(int themeResId) {
        mTheme = themeResId;
        if (mTheme != -1) {
        	TypedArray a = getContext().getApplicationContext().obtainStyledAttributes(mTheme, R.styleable.PickersDialogFragment);

            mTextColor = a.getColor(R.styleable.PickersDialogFragment_textColor, mTextColor);
            mPrimaryTextSize = a.getDimension(R.styleable.PickersDialogFragment_primaryTextSize, mLineHeight);
            mPrimaryTextColor = a.getColor(R.styleable.PickersDialogFragment_primaryTextColor, mPrimaryTextColor);
            mPrimaryTextColorBold = a.getColor(R.styleable.PickersDialogFragment_primaryTextColorBold, mPrimaryTextColorBold);
            
            mSecondaryTextSize = a.getDimension(R.styleable.PickersDialogFragment_secondaryTextSize, mSecondaryTextSize);
            mSecondaryTextColor = a.getColor(R.styleable.PickersDialogFragment_secondaryTextColor, mSecondaryTextColor);
            mSecondaryTextColorBold = a.getColor(R.styleable.PickersDialogFragment_secondaryTextColorBold, mSecondaryTextColorBold);
            
            mLineHeight = a.getFloat(R.styleable.PickersDialogFragment_lineHeight, 2.0f);
            
            rightShadow = a.getDrawable(R.styleable.PickersDialogFragment_shadowRight);
            leftShadow = a.getDrawable(R.styleable.PickersDialogFragment_shadowLeft);
            a.recycle();
        }
    ...
    private void restyleViews() {
        for (int i = 0; i < childCount; i++) {
        	...
        	if(view instanceof TimeTextView){
        		((TimeTextView)view).setupView(i == centerIndex, getBundle());
        	}
        	else if(view instanceof TimeLayoutView){
        		((TimeLayoutView)view).setupView(getContext(), i == centerIndex, getBundle());
        	}
        	else if(view instanceof DayTimeLayoutView){
        		((DayTimeLayoutView)view).setupView(getContext(), i == centerIndex, getBundle());
        	}
        }
    }
    ...
    protected void onFinishInflate() {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            TimeView ttv = mLabeler.createView(getContext(), i == centerIndex, getBundle());
            addView((View)ttv, lp);
        }
